...
contract RockPaperSatoshi {
    string public constant contractName = "RockPaperSatoshi";

    RPSatoshiToken public immutable RPSatoshi;//This is our general currency (ERC20) token contract.
    RPSHashHealToken public immutable RPSHashHeal;//This is our common healing item (ERC20) token contract.
    RPSRareItemsToken public immutable RPSRareItems;//This is our (ERC721) contract for minting and keeping track of unique/rare items the player can obtain.
    ...
    function battlePvE(uint256 move_) public onlyRegistered {//Function for the player to battle the bot encountered with either a rock, paper, or scissors. Possibly outcomes are draw, win, lose, and loss absorbed.
        ...
        if ((botMove == 0 && move_ == 1) || (botMove == 1 && move_ == 2) || (botMove == 2 && move_ == 0)) {//All scenarios where the player beats the bot.
            ...
            if (((uint256(keccak256(abi.encodePacked(block.timestamp,msg.sender,botMove,move_)))) % 2) == 1) {//psudo-random-number generator for if player receives a hash heal for their win. (50% chance of getting a hash heal).
                RPSHashHeal.mint(msg.sender, 1 * 10**RPSHashHeal.decimals());//Mint 1 hash heal to the player.
            }
    ...
    function equipRareItem(uint256 tokenId_) public onlyRegistered notInEncounter {//Function for the player to equip rare items that they own.
        ...
        for (uint256 i = 0; i < RPSRareItems.balanceOf(msg.sender); i++) {//Loop through all the rare items the player ownes and check that they don't already have the max of 2 equipped. Initialize the loop iterations to the banlanceOf (aka # of) rare items the player owns.
        ...
        //Make is so they cannot trade during combat.
            uint256 currentLossAbsorbMaxSum = 0;//Will be used to calculate/sum the loss absorb gained from the 1-2 rare items the player has equipped.
            uint256 incomeForWinBonusSum_ = 0;//Will be used to calculate the income from winning bonus gained from the 1-2 rare items the player has equipped.
            uint256 maxHealthIncreaseModifierSum_ = 0;//Will be used to calculate the max income from winning bonus gained from the 1-2 rare items the player has equipped.
            uint256 healthCostToUseSum_ = 0;//Will be used to calculate the rare item health cost from the 1-2 rare items the player has equipped.

            for (uint256 i = 0; i < RPSRareItems.balanceOf(msg.sender); i++) {//Initialize the loop iterations to the banlanceOf (aka # of) rare items the player owns.
                uint256 tokenIdToCheck = RPSRareItems.tokenOfOwnerByIndex(msg.sender,i);//Use tokenOfOwnerByIndex to get the token ID of each rare item owned by the user.
                RPSRareItemsToken.RareItemAttributes memory rareItemAttributesFromToken = RPSRareItems.readAttributesByTokenId(tokenIdToCheck);//Use the readAttributesByTokenId function to fill the rareItemAttributesFromToken struct with the current rare item from the loop.
                if(rareItemAttributesFromToken.equipped){//Check if the rare item is equipped.
